// src/pages/Cards.jsx  (your file name might be Card.jsx; keep your original export name)
    ...
    <div className="bg-gradient-to-tr from-white via-pink-50 to-purple-50 dark:from-slate-800 dark:via-slate-900 dark:to-gray-900 shadow-lg rounded-2xl overflow-hidden flex flex-col justify-between border border-gray-200 dark:border-gray-700">
      ...
      <div className="p-6 flex-1 flex flex-col justify-between space-y-3">
        ...
        <div className="flex flex-col gap-2 mt-4">
          ...
          >
            {isEnrolled ? "📘 In My Learning" : "Start Learning"}
          </button>

          {/* ✅ Route to BOOK details, not course. Also pass the item via state for instant render */}
          <Link
  to={data.title ? (data.price ? `/course/${data._id}` : `/book/${data._id}`) : "#"}
  className="mt-3 px-4 py-2 rounded-lg font-semibold bg-gradient-to-r from-indigo-600 to-purple-600 
             text-white hover:opacity-90 transition inline-block text-center"
>
  🔍 View Detail
</Link>
        </div>
